...
import {handleExternalSignature} from "./order021-external-signature.js";
...
export default{async fetch(request,env,ctx){const url=new URL(request.url);const boundary=await enforceOwnerMutationBoundary(request);if(boundary)return boundary;const owner=await handleOwnerInbox(request,env);if(owner)return owner;const externalSignature=await handleExternalSignature(request,env);if(externalSignature)return externalSignature;const handled=await handleHumanGateRequest(request,env,ctx);if(handled)return handled;if(url.pathname==="/"&&(request.method==="GET"||request.method==="HEAD"))return dashboard(request,env,ctx);if(url.pathname==="/api/status"&&request.method==="GET")return status(request,env,ctx);return x402.fetch(request,env,ctx)},async scheduled(controller,env,ctx){return x402.scheduled(controller,env,ctx)}};
